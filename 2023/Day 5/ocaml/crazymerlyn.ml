(* Range and parse_numbers are in a Myutils module file normally and imported  open Myutils*)
...
let input_lines = Core.In_channel.input_lines stdin
let seeds = parse_numbers (List.nth (String.split_on_char ':' (List.hd input_lines)) 1)
let maps = get_maps (List.tl (List.tl input_lines))
